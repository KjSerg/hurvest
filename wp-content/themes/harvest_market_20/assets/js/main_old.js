...
function initMap() {
    var mapElement = document.getElementById('map-product1'),
        ...
        function addMarker(data, map) {
            ...
            $.each(description, function () {
                $.each(this, function (name, value) {
                    var htm_ = '<li><span>' + name + '</span> <strong>' + value + '</strong></li>';
                    text_.push(htm_);
                });
            });
            var str_ = "";
            $.each(text_, function (e) {
                str_ += text_[e];
            });
            var img_ = [];
            $.each(slider, function (e) {
                var img_html = '<div><img src="' + this + '" alt=""></div>';
                img_.push(img_html);
            });
            var str_slider = "";
            $.each(img_, function (e) {
                str_slider += img_[e];
            });
            var verified_html = "";
            ...
            var content = '<div class="iw-wrap">' + '<div class="iw-container">' + '<div class="iw-product-media">' + '<div class="product-item__date">' + date_ + '</div>' + '<a class="product-item__favorite" href="#"><svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve" style="enable-background:new 0 0 659.3 578.6" viewBox="0 0 659.3 578.6"><path d="m78 325 231.8 217.7c8 7.5 12 11.2 16.7 12.2 2.1.4 4.3.4 6.4 0 4.7-.9 8.7-4.7 16.7-12.2L581.3 325c65.2-61.3 73.1-162.1 18.3-232.7L589.3 79C523.7-5.6 392 8.6 345.9 105.2c-6.5 13.6-25.9 13.6-32.4 0C267.4 8.6 135.7-5.6 70.1 79L59.7 92.3C4.9 163 12.8 263.8 78 325z" style="fill:none;stroke:#fff;stroke-width:46.6667;stroke-miterlimit:133.3333"></path></svg></a>' + '<div class="product-item__slider">' + str_slider + '</div>' + '</div>' + '<div class="iw-content">' + '<a class="product-item__title" href="' + link + '">' + title + '</a>' + '<div class="iw-product-subtitle">' + subtitle + '</div>' + '<div class="iw-product-provider">' + '<div class="iw-product-delivery">' + delivery + '</div>' + verified_html + '</div>' + '<ul class="iw-product-place"> <li>' + place + '</li><li>' + distance + '</li></ul>' + '<ul class="iw-product-description">' + str_ + '</ul>' + '<div class="product-item__bot">' + '<div class="product-item__price"> <strong>' + price + ' </strong>' + price_value + '</div>' + '<ul class="product-item__reviews"><li>' + reviews + '</li> <li> <strong>' + rating + '</strong> <svg xmlns="http://www.w3.org/2000/svg" xml:space="preserve" style="enable-background:new 0 0 12 11.2" viewBox="0 0 12 11.2"><path d="M12 4.2c-.1-.2-.3-.4-.5-.4L8 3.5 6.6.4C6.5.1 6.3 0 6 0s-.5.1-.6.4L4 3.5l-3.4.3c-.3 0-.5.2-.6.4 0 .3 0 .5.2.7l2.6 2.2-.8 3.3c-.1.2 0 .5.2.6.1.1.2.1.4.1.1 0 .2 0 .3-.1l3-1.7 3 1.7c.2.1.5.1.7 0 .2-.1.3-.4.2-.6l-.6-3.3 2.6-2.2c.2-.2.2-.4.2-.7z" style="fill:#ffc327"></path></svg></li>' + '</ul>' + '</div>' + '</div>' + '</div>' + '</div>';
            ...
            $(document).on('click', function (e) {
                if (!$(e.target).closest('.marker-price, .iw-wrap').length) {
                    $('.marker-price').removeClass("active");
                    infowindow.close();
                }
