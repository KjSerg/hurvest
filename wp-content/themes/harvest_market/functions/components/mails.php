...
function send_notification( $user_id, $ID ) {
	...
	$permalink         = get_edit_post_link( $ID );
	$form_subject      = "Оголошення потребує модерацію";
	$user_form_subject = "Ваше оголошення відправлене на модерацію";
	$string_1          = 'Заголовок оголошення';
	$string            = 'ID оголошення';
	$string1           = $ID;
	$string2           = 'Оголошення';
	...
	$message           .= "
		...
		<td style='padding: 10px; border: #e9e9e9 1px solid;' > $string1</td>
		</tr>
		  ";
	$message           .= "
		" . ( ( $c = ! $c ) ? '<tr>' : '<tr style="background-color: #f8f8f8;">' ) . "
	...
	$message_admin     = "
		...
		<td style='padding: 10px; border: #e9e9e9 1px solid;' > $string3</td>
		  ...
		  ";
	$message_admin     = "<table style='width: 100%;'> $message_admin</table> ";
	$headers           = "MIME-Version:1.0" . PHP_EOL .
	                     "Content-Type:text/html; charset=utf-8" . PHP_EOL .
	                     'From:' . ___adopt( $project_name ) . ' <new_advertisement@' . $_SERVER['HTTP_HOST'] . '>' . PHP_EOL .
	                     'Reply-To: ' . $admin_email . '' . PHP_EOL;
	wp_mail( $email, $user_form_subject, $message, $headers );
	wp_mail( $admin_email, $form_subject, $message_admin, $headers );
}
...
function send_message( $m, $email = false, $form_subject = 'Повідомлення із сайту' ) {
	$c            = true;
	$message      = $m;
	$project_name = get_bloginfo( 'name' );
